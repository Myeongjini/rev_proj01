...
| 11 | AK | Skill Auto-Unlock by Level | 🔴 TODO | AJ ✅ |
...
### ✅ Definition of Done
- [ ] Unity Console clean
- [ ] `SkillDefinition` 에 `unlockLevel: int` 필드 추가
- [ ] 시드값:
  - meteor.unlockLevel = 1
  - cold_beam.unlockLevel = 5
  - charge.unlockLevel = 10
- [ ] `SkillCastOrchestrator.EquipSkill(slotIndex, skillId)` 가 PlayerLevel 검사:
  - 현재 레벨 < skill.unlockLevel → return false + 피드백 "Lv N 필요"
- [ ] `SkillBarSlotView` 의 슬롯이 잠금 상태일 때 자물쇠 아이콘 + "Lv N" 라벨 표시
- [ ] `SkillTabPanel` 의 카드도 잠금 상태 시각화 — 어두운 오버레이 + 자물쇠 아이콘 + "Lv N 필요" 라벨, 장착 버튼 비활성
- [ ] PlayerLevelService.LevelChanged 이벤트 구독 → 새 레벨에서 해금되는 스킬이 있으면 알림 popup ("새 스킬: 콜드빔 해금됨!")
- [ ] 자동 슬롯 배치: 레벨 도달로 신규 해금된 스킬이 자동으로 빈 슬롯에 장착되는지 → **No, 사용자가 직접 장착해야 함** (Bundle 8 Y 의 장착 버튼 흐름 유지)
