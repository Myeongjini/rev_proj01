...
### ✅ Definition of Done
- [x] Unity Console clean
- [x] `Assets/Scenes/EXPDungeonScene.unity` 신규 (GoldDungeonScene 복제 + EXP 보상 모드)
- [x] Build Settings: `[3] EXPDungeonScene`
- [x] **MainUI01 진입점:** 현재 "골드던전" 이 5번 슬롯. EXP 던전 진입은 별도 위치 고민 필요 — 권장: GoldDungeonEntryPanel 의 "EXP 던전" 탭으로 합치는 방식 (한 모달 내 두 던전 선택)
- [x] `EXPDungeonService` MonoBehaviour — GoldDungeonService 패턴 복사 + EXP 보상으로 변경
- [x] EXP 보상: 처치당 EXP × 3.0 multiplier (시드값)
- [x] 일일 입장 3회 (시드값) — GoldDungeon 과 별도 카운터
- [x] Difficulty Lv1~Lv5 — Lv1 활성, Lv2~Lv5 는 PlayerLevel unlockLevel 기반 잠금
- [x] Sweep + 광고 2배 모두 지원
- [x] EXP 보상 받기 → PlayerLevelService.GrantExp(N) → 레벨업 가능 (기존 메커니즘 사용)
