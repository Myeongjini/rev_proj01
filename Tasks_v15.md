...
| 2026-05-12 | BA | Added EnhancementService, EnhancementCostCalculator, `enhanceItem` callable, enhancement levels on weapon/armor/accessory owned entries, save document mapping, equipped stat enhancement multipliers, and fusion-compatible level reset behavior through aggregate inventory entries. Validation: `dotnet build Assembly-CSharp.csproj --no-restore` passed with 4 pre-existing Chat/Presence warnings; `npm --prefix functions run build` passed; Unity PlayMode test job `25b27b70bf324a87a33c86dd2a80c8bd` passed with 0 discovered tests. |
| 2026-05-12 | BB | Added EnhancementModal, enhancement success VFX prefab, inventory detail enhancement buttons, slot `+N` display support, and GameContext/GameManager/HUD wiring for the modal/service. Validation: `dotnet build Assembly-CSharp.csproj --no-restore` passed with 4 pre-existing Chat/Presence warnings; Unity Console had no script compile errors after refresh; Unity PlayMode test job `aba8b67917624c0bacd63208e9c4f1b3` passed with 0 discovered tests. |
| 2026-05-12 | BC | Bumped SaveData default schema to v11 and added v10→v11 default-fill/normalization for enhancement levels, enhancement stone, and EnhancementStoneDungeon state. Validation: `dotnet build Assembly-CSharp.csproj --no-restore` passed with 4 pre-existing Chat/Presence warnings; `npm --prefix functions run build` passed; Unity Console had no script compile errors after refresh. PlayMode rerun was not available because the tool returned a usage-limit rejection. |
