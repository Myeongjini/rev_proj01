...
| 2026-05-13 | BE | Added Firestore rules for `/rankings/combatPower/entries/{uid}` with authenticated read, owner-only create/update, strict score/displayName/lastUpdateUtcMs validation, and delete denial. Added descending and ascending `entries` composite indexes for TOP 100 and surrounding-rank queries. Validation: Firestore database detected as `(default)` STANDARD/FIRESTORE_NATIVE; `firestore.indexes.json` parsed successfully; `dotnet build Assembly-CSharp.csproj --no-restore` passed with 4 pre-existing Chat/Presence warnings. Deploy validation was not run because AGENTS requires user-run `firebase deploy`. |
| 2026-05-13 | BE follow-up | Fixed LoginScene Firestore permission regression by adding explicit owner-only `/users/{uid}/profile/main` rules with profile field validation. Unity Console showed LoginBootstrap failing at `UserProfileService.GetOrCreateAsync` with `Missing or insufficient permissions`; this was caused by profile subcollection rules not being defined. Validation: `dotnet build Assembly-CSharp.csproj --no-restore` passed with 4 pre-existing Chat/Presence warnings. |
