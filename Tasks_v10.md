...
| 2026-05-11 | Task AH | Added `GoldDungeonResultModal`, `GoldDungeonDifficultySlotView`, result/difficulty prefabs, SceneTransfer pending result flow, EntryPanel difficulty slots + Sweep, Nanum TMP font reuse for generated gold dungeon labels, and GameManager/GameContext result modal binding into the startup popup queue after offline rewards. Validation PASS: `dotnet build Assembly-CSharp.csproj --no-restore` 0 errors / 4 pre-existing Chat/Presence DTO warnings; MCP validation confirmed difficulty slot lock state, countdown label, pending result auto modal, normal/ad 2x reward math, best record update, sweep prerequisites, daily entry cap, and offline-before-gold popup queue order. Unity Console residual entries are MCP stale-client disconnect logs only; no Task AH product error was found. |
| 2026-05-11 | Task AI | Bumped default `SaveData.saveVersion` to 6 and added explicit v5→v6 `goldDungeon` default-fill migration while preserving existing v6 gold dungeon values. Firestore mirror required no further code change because AG already added `SaveDataDocument.GoldDungeon` plus mapper round-trip. Validation PASS: `dotnet build Assembly-CSharp.csproj --no-restore` 0 errors / 4 pre-existing Chat/Presence DTO warnings; MCP validation confirmed v5→v6 default-fill, v6 no-double-migration preservation, Firestore document/mapper round-trip, GoldDungeonScene/build/prefab presence, daily 3-entry cap, normal/ad 2x reward math, best score persistence, and cross-feature API sanity for popup queue, KST reset, attendance, skills, gacha 30-pull, chat, and presence. Unity Console residual entries are MCP stale-client disconnect logs only; no Task AI product error was found. |
